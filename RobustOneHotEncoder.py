...
class RobustOneHotEncoder:
    ...
    def fit(self,df,cat_columns, prefix_sep = '__', sparse = False, dummy_na = True,**getdummiesargs):
        if cat_columns.__class__ == str:
    ...
    def transform(self, df, verbose = True, keep_cols = True,sparse = False, dummy_na = True, return_new_columns = False):
        if keep_cols == True:
            col_dict = {col:df[col] for col in df}
        ...
        one_hot_transform = one_hot_transform.loc[:,~one_hot_transform.columns.duplicated()]

        if keep_cols == True:
            one_hot_transform = one_hot_transform.assign(**col_dict)
